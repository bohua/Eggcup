...
(function ($) {
	$.widget("metro.calendar", {
		...
		_create: function () {
			...
			if (element.data('selectMode') != undefined) this.options.startMode = element.data('selectMode');
		...
		_initButtons: function () {
			...
			if (this._mode == 'day') {
				...
				table.find('.month a').on('click', function (e) {
						...
						if (that.options.selectMode === 'day') {
							that._mode = 'day';
							that._renderCalendar();
						} else if (that.options.selectMode === 'month') {
							var currentMonth = (new Date(that._year, that._month, 1)).format(that.options.format, null);
							that.options.click(currentMonth, -1);
						}
					}
				)
				;
